...
class MAAgent(parl.Agent):
    ...
    def learn(self, agents):
        ...
        for i in range(self.n):
            ...
            noise = np.random.normal(0, self.policy_noise, size=target_act_next.shape)
            noise = paddle.clip(paddle.to_tensor(noise), -self.noise_clip, self.noise_clip)
        ...
        target_q_next1, target_q_next2 = self.alg.Q(
            batch_obs_next_n, target_act_next_n, use_target_model=True)
        target_q_next = paddle.minimum(target_q_next1, target_q_next2)
